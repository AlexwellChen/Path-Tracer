...
vec3 Li(Ray& primary_ray)
{
	...
	//	Ray hit2light = point_light.position - (hit.position + EPSILON);
	//	if (occluded(hit2light)) {
	//		return L;
	//	}
	...
	for (int bonces = 0; bonces < settings.max_bounces; bonces++) {
		...
		LinearBlend metal_blend(hit.material->m_metalness, &metal, &dielectric); 
		LinearBlend reflectivity_blend(hit.material->m_reflectivity, &metal_blend, &diffuse);
		BRDF & mat = metal_blend;

		
		shadowRay = Ray();
		shadowRay.d = point_light.position - (hit.position + vec3(EPSILON));
		...
		//wi = vec3(0.0f); // random sample vector
		float p = 0.0f;
